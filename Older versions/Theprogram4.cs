/*The fourth version of Theprogram.cs.
	...
	public List<string> get_amta_name_with_offset(ref byte[] data, ref string file_name_ext, int i, ref uint amta_count) //this is a method to get the AMTA tags based on the available offset
	{
		...
		for (amta_number = 0; amta_number < amta_count;)
		{
			if (data[i] == 65 && data[i + 1] == 77 && data[i + 2] == 84 && data[i + 3] == 65)
			{
				if (data[i + 7] == 5)
				{
					amta_length_container.SetValue(data[i + 11], 0);
					amta_length_container.SetValue(data[i + 10], 1);
					amta_length_container.SetValue(data[i + 9], 2);
					amta_length_container.SetValue(data[i + 8], 3);

					amta_offset_name_start.SetValue(data[i + 39], 0);
					amta_offset_name_start.SetValue(data[i + 38], 1);
					amta_offset_name_start.SetValue(data[i + 37], 2);
					amta_offset_name_start.SetValue(data[i + 36], 3);
